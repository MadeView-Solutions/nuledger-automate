...
-- Create settlement item type enum if not exists
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_settlements_organization ON public.settlements(organization_id);
CREATE INDEX IF NOT EXISTS idx_settlements_case ON public.settlements(case_id);
...
CREATE INDEX IF NOT EXISTS idx_disbursement_templates_org ON public.disbursement_templates(organization_id);
CREATE INDEX IF NOT EXISTS idx_disbursement_template_lines_template ON public.disbursement_template_lines(template_id);
...
DROP POLICY IF EXISTS "Users can view time entries in their organizations" ON public.time_entries;
...
DROP POLICY IF EXISTS "Users can view settlements in their organizations" ON public.settlements;
CREATE POLICY "Users can view settlements in their organizations"
ON public.settlements FOR SELECT
USING (organization_id IN (SELECT user_organizations(auth.uid())));

DROP POLICY IF EXISTS "Staff can manage settlements" ON public.settlements;
...
DROP POLICY IF EXISTS "Users can view settlement items in their organizations" ON public.settlement_items;
...
DROP POLICY IF EXISTS "Staff can manage settlement items" ON public.settlement_items;
...
DROP POLICY IF EXISTS "Users can view templates in their organizations" ON public.disbursement_templates;
CREATE POLICY "Users can view templates in their organizations"
ON public.disbursement_templates FOR SELECT
USING (organization_id IN (SELECT user_organizations(auth.uid())));

DROP POLICY IF EXISTS "Admins can manage templates" ON public.disbursement_templates;
...
DROP POLICY IF EXISTS "Users can view template lines in their organizations" ON public.disbursement_template_lines;
...
DROP POLICY IF EXISTS "Admins can manage template lines" ON public.disbursement_template_lines;
...
DROP TRIGGER IF EXISTS update_disbursement_templates_updated_at ON public.disbursement_templates;
...
DROP MATERIALIZED VIEW IF EXISTS public.mv_perf_weekly CASCADE;
...
CREATE OR REPLACE FUNCTION public.fn_apply_disbursement_template(_settlement_id UUID, _template_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public

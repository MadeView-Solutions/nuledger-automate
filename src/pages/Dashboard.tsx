...
import { SearchIcon, BellIcon, Menu, Settings, LogOut, User, HelpCircle, Home, BarChart3, FileText, CreditCard, Users, PieChart } from "lucide-react";

const navigation = [
  { name: "Dashboard", href: "/dashboard", icon: Home, current: true },
  { name: "Transactions", href: "/transactions", icon: BarChart3, current: false },
          ...
          <div className="flex-1 overflow-y-auto py-5 px-4">
            <nav className="space-y-1">
              {navigation.map((item) => (
                ...
                >
                  <item.icon
                    className={cn("mr-3 h-5 w-5 flex-shrink-0", item.current ? "text-primary-foreground" : "text-muted-foreground group-hover:text-foreground")}
                  />
                  {item.name}
                </Link>
              ))}

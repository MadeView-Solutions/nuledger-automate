...
export const parseFinancialEntry = async (transcript: string): Promise<FinancialEntry> => {
  ...
  if (lowerTranscript.includes("transfer") || lowerTranscript.includes("move")) {
    ...
    const expenseCategories = extractExpenseCategory(lowerTranscript);
    ...
    const incomeCategories = extractIncomeCategory(lowerTranscript);
    if (incomeCategories) {
  ...
  let description = generateDescription(type, category, source, destination);
...
// Helper function to extract expense categories
...
// Helper function to extract income categories
...
// Helper function to extract date from transcript
function extractDate(transcript: string): Date | undefined {
  ...
  for (const month of months) {
    if (transcript.includes(month)) {
      ...
      const dayMatch = transcript.substr(transcript.indexOf(month)).match(/\d+/);

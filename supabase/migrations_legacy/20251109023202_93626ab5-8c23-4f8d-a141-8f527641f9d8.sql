...
DROP TRIGGER IF EXISTS validate_journal_balance_trigger ON public.journal_lines;
...
DROP POLICY IF EXISTS "Admins and owners can manage members" ON public.org_members;
...
DROP POLICY IF EXISTS "Users can view journal lines in their organizations" ON public.journal_lines;
...
-- Storage policies (drop if exist, then create)
DROP POLICY IF EXISTS "Users can view attachments in their organizations" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can view exports in their organizations" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can view templates in their organizations" ON storage.objects;
